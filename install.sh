#!/usr/bin/env bash
# install.sh — install the `landed` Claude Code skill.
# Usage:
...
#   bash install.sh --hook         # also wire up the proactive Stop hook
#   bash install.sh --uninstall    # remove the install
set -euo pipefail

REPO_URL="https://github.com/pedroreisper/landed.git"
SKILL_NAME="landed"
...
printf '\n\033[32m✓\033[0m landed installed at %s\n' "$TARGET"
printf '\nNext steps:\n'
...
if [ "$WIRE_HOOK" -eq 0 ]; then
  printf '  • Enable proactive blocking (recommended):  bash %s/hooks/install_hook.sh\n' "$TARGET"
fi
printf '  • Manual invocation: /landed  (or trigger phrases — see SKILL.md)\n'
